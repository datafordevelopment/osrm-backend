                     ...
                     const std::vector<extractor::QueryNode> &query_nodes)
{
    ...
    const bool canBeDiscretized = [&]() {
        ...
        DiscreteBearing last_discrete_bearing =
            util::guidance::BearingClass::getDiscreteBearing(turns.back().bearing);
        for (const auto turn : turns)
        {
            const DiscreteBearing discrete_bearing =
                util::guidance::BearingClass::getDiscreteBearing(turn.bearing);
    ...
    if (canBeDiscretized)
    {
        for (const auto turn : turns)
        {
            ...
            bearing_class.add(util::guidance::BearingClass::getDiscreteBearing(turn.bearing) *
    ...
    else
    {
        for (const auto turn : turns)
        {
            if (turn.entry_allowed)
                entry_class.activate(number);
            bearing_class.add(turn.bearing);
            ++number;
        }
    }
