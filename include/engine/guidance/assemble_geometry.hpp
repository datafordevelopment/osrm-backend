                             ...
                             const PhantomNode &target_node)
{
    ...
    for (const auto &path_point : leg_data)
    {
        ...
        geometry.progression.emplace_back(LegGeometry::ProgressStep{current_distance, path_point.duration_until_turn / 10.});
    ...
    geometry.segment_distances.push_back(cumulative_distance);
    geometry.progression.emplace_back(LegGeometry::ProgressStep{current_distance, target_node.forward_weight / 10.});
    geometry.segment_offsets.push_back(geometry.locations.size());
    geometry.locations.push_back(target_node.location);

    BOOST_ASSERT(geometry.segment_distances.size() == geometry.segment_offsets.size() - 1);
    BOOST_ASSERT(geometry.locations.size() > geometry.segment_distances.size());
    BOOST_ASSERT(geometry.progression.size() == geometry.locations.size() - 1);
